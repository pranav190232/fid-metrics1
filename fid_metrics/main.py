...
#import sys
#sys.path.append(r"C:\Users\prana\OneDrive\Desktop\fid_metrics")
...
def build_loaders(type, paths, cfg):
    ...
    for path in paths:
        ...
        if is_video_path(path):
            print(f"Identified as video: {path}")  # Debug print
            if type == 'fid':
                if dataset_cfgs:
                    dataset_cfgs = dict(dataset_cfgs)
                    dataset_cfgs['sequence_length'] = bs
            ...
            print(f"Identified as image directory: {path}")  # Debug print
            C = ImageDataset if type == 'fid' else ImageSequenceDataset
        else:
            raise NotImplementedError(f"Path {path} is not recognized as a valid video or image directory.")
